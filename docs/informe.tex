...
\begin{document}
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|l|X|}
			...
			\hline
			\textbf{RF-01} & Registrar mascota & Permitir al propietario registrar una mascota ingresando su información general, incluyendo nombre, especie, raza, sexo, edad, estado reproductivo y fotografías para su identificación dentro de la plataforma. \\ \hline
			\textbf{RF-02} & Gestionar historial médico & Permitir registrar y actualizar el historial médico de una mascota, incluyendo vacunas, desparasitaciones, enfermedades, alergias, cirugías, tratamientos y controles veterinarios. \\ \hline
			\textbf{RF-03} & Consultar perfil completo & Permitir visualizar el perfil completo de una mascota mostrando su información general, historial médico autorizado, fotografías, comportamiento, vacunas, edad, raza para procesos de adopción o cruza. \\ \hline
			\textbf{RF-09} & Validar información médica & Permitir que una veterinaria autorizada valide la información médica registrada de una mascota, indicando que los datos han sido verificados por un profesional. \\ \hline
			\textbf{RF-14} & Gestionar carnet de vacunación & Permitir registrar, actualizar y consultar el carnet de vacunación de cada mascota, mostrando el historial de vacunas aplicadas, fechas, próximas dosis y su estado de vigencia. \\ \hline
			\textbf{RF-16} & Gestionar antecedentes genéticos & Permitir registrar y consultar los antecedentes genéticos y el grado de parentesco de una mascota, incluyendo linaje para procesos de cruza responsable. \\
			\hline
		\end{tabularx}
	\end{table}
	...
	La pestaña \textit{C++[3]} del generador confirmó las plantillas exactas que BOUML aplica según el tipo de relación: un atributo simple se declara como \${type} \${name}\${value};, mientras que las relaciones de asociación, agregación y composición con multiplicidad múltiple (*) se declaran anteponiendo un asterisco al nombre (\${type} * \${name}\${value};), es decir, mediante un puntero simple y no mediante un contenedor STL por defecto. En consecuencia, la asociación \textit{Propietario} (1) $\leftrightarrow$ \textit{Mascota} (0..*) se mapeó explícitamente a vector<Mascota*> mascotas; en Propietario.h, sustituyendo la plantilla nativa de puntero simple por el tipo contenedor vector definido en la pestaña \textit{C++[5]}, que ya incluye su \#include <vector> y using namespace std; correspondientes. Las operaciones de acceso (\textit{getters}/\textit{setters}) se generaron siguiendo la plantilla de la pestaña \textit{C++[4]}: los métodos get\_\${name} se generan públicos, inline y const, mientras que los set\_\${name} se generan públicos sin modificadores adicionales, motivo por el cual el equipo ajustó manualmente la visibilidad y el calificador const de algunas operaciones críticas del dominio dentro de los bloques protegidos.
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Asociación 1--N Mascota (1) $\leftrightarrow$ EvaluacionVeterinaria (0..*) & Contenedor vector<EvaluacionVeterinaria *> evaluaciones; en Mascota.h \\ \hline
			Asociación 1--N Veterinario (1) $\leftrightarrow$ EvaluacionVeterinaria (0..*) & Contenedor vector<EvaluacionVeterinaria *> evaluacionesRegistradas; en Veterinario.h \\ \hline
			Clase asociativa MascotaVacuna & MascotaVacuna.h / MascotaVacuna.cpp, referenciada desde Vacuna.h mediante vector<MascotaVacuna *> aplicaciones; \\ \hline
		\end{tabularx}
	\end{table}
	
	\subsection{Limitaciones detectadas}
	Durante el proceso de generación y verificación se identificaron cuatro limitaciones concretas del generador C++ de BOUML, todas corregidas manualmente antes de lograr una compilación exitosa:
	\begin{itemize}
		...
		\item \textbf{Ausencia de directivas de inclusión automáticas para tipos contenedor.} El tipo vector no generó automáticamente su directiva de inclusión correspondiente en todos los archivos de cabecera, requiriendo verificación manual.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\textwidth]{figuras/despues_modelo.png}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.65\textwidth]{figuras/diff_codigo.png}
	...
	\begin{itemize}[leftmargin=*]
		...
		\begin{itemize}
			\item \textit{Foco de contribución técnica:} Fundamentación teórica de las transformaciones M2M/M2T, metamodelos y plantillas del marco conceptual; parametrización global del motor de generación física (rutas de salida, extensiones .h/.cpp, guardas de inclusión) y ajuste de multiplicidades múltiples mediante contenedores \texttt{std::vector}; codificación de la unidad demostrable \texttt{main.cpp} y pruebas de compilación nativa con g++.
			\item \textit{Historial completo de Commits:} \\
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.85\textwidth]{figuras/github_contributors.png}
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Registrar una mascota ingresando su información general, incluyendo nombre, especie, raza, sexo, edad, estado reproductivo y fotografías para su identificación dentro de la plataforma. \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Registrar y actualizar el historial médico de una mascota, incluyendo vacunas, desparasitaciones, enfermedades, alergias, cirugías, tratamientos y controles veterinarios. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Visualizar el perfil completo de una mascota mostrando su información general, historial médico autorizado, fotografías, comportamiento, vacunas, edad, raza para procesos de adopción o cruza. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Permitir que una veterinaria autorizada valide la información médica registrada de una mascota, indicando que los datos han sido verificados por un profesional. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Registrar, actualizar y consultar el carnet de vacunación de cada mascota, mostrando el historial de vacunas aplicadas, fechas, próximas dosis y su estado de vigencia. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Registrar y consultar los antecedentes genéticos y el grado de parentesco de una mascota, incluyendo linaje para apoyar los procesos de cruza responsable. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Buscar mascotas aplicando filtros como especie, raza, edad, ubicación, estado de salud y disponibilidad para adopción o cruza. \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Permitir que un usuario interesado envíe solicitudes de adopción y que el propietario pueda aceptarlas o rechazarlas. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Evaluar la compatibilidad entre dos mascotas registradas para procesos de cruza considerando raza, edad, salud y linaje genético. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Habilitar un canal de comunicación entre dos usuarios registrados cuando exista un proceso de adopción o cruza activo. \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Permitir que los propietarios soliciten procesos de cruza entre mascotas y que el receptor pueda aceptar o rechazar la solicitud. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Enviar un recordatorio al propietario de la mascota antes de la fecha programada de una cita veterinaria y registrar la notificación. \\ \hline
			Prioridad (MoSCoW) & Should \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Permitir que el propietario determine qué información de su mascota y de su perfil podrá ser visualizada por otros usuarios. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Validar la información obligatoria proporcionada por el usuario antes de aprobar el proceso de verificación de identidad. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Permitir al propietario publicar una mascota indicando si estará disponible para adopción, cruza o ambas opciones. \\ \hline
			Prioridad (MoSCoW) & Must \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Permitir al propietario consultar el historial de solicitudes y procesos de adopción y cruza asociados a sus mascotas. \\ \hline
			Prioridad (MoSCoW) & Could \\ \hline
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|l|X|}
			...
			Descripción & Validar las imágenes cargadas por los usuarios verificando que cumplan con las políticas de contenido de la plataforma. \\ \hline
			Prioridad (MoSCoW) & Should \\ \hline
...
\begin{longtable}{|p{3.5cm}|p{3.5cm}|p{4.2cm}|p{3.8cm}|}
	...
	Asociación Propietario $\leftrightarrow$ Mascota & Cardinalidad 0..* & vector<Mascota*> mascotas; & Propietario.h \\ \hline
	...
	Vínculo MascotaVacuna $\to$ Mascota & Navegación Relacional & Mascota* mascota; & MascotaVacuna.h \\ \hline
	Vínculo MascotaVacuna $\to$ Vacuna & Navegación Relacional & Vacuna* vacuna; & MascotaVacuna.h \\ \hline
	Asociación Vacuna $\leftrightarrow$ MascotaVacuna & Cardinalidad Múltiple & vector<MascotaVacuna*> aplicaciones; & Vacuna.h \\ \hline
	
	Clase EvaluacionVeterinaria & <<Entity>> & class EvaluacionVeterinaria \{ & EvaluacionVeterinaria.h \\ \hline
	Asociación Mascota $\to$ Evaluación & Unidireccional 0..* & vector<EvaluacionVeterinaria*> evaluaciones; & Mascota.h \\ \hline
	Asociación Veterinario $\leftrightarrow$ Evaluación & Bidireccional Simple 0..* & vector<EvaluacionVeterinaria*> evRegistradas; & Veterinario.h \\ \hline
\end{longtable}
